...
# TO DO: Nanme is generated by LLM but is used by graphs to link events. Maybe create an ID based on the user_prompt?

class HistoricalPathwaySimulator:
    max_iterations = 0
    count = 0

    def __init__(self, config, main_prompt_content, sub_event_prompt_content, random_mode, test_mode):
        self.config = config
